...
namespace Enochian_Encryption_System
{
    public partial class FrmRegeneration : Form
    {
        ...
        {
            if (!GlobalSession.DecStep4_Done) { MessageBox.Show("Complete Step 4 first."); return; }
        ...
        private void btnReconstruct_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // No Loop Needed anymore because Deterministic logic never fails.
                int keyFactor;
                ...
                int gcdValue = (int)BigInteger.GreatestCommonDivisor(BigInteger.Abs(validDet), 21);
        ...
        // [FIX] EXACT COPY of Deterministic Logic
        private int[,] GenerateDeterministicMatrix(int N, int masterSeed, out int factor)
        {
            ...
            for (int k = 0; k < N * 5; k++)
            {
                // New RNG for EVERY step ensures lock-step synchronization
